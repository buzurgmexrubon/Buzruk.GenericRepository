...
/// <summary>
/// This class the logic for filtering, sorting, and eager loading of related entities.
/// It takes an IQueryable<T> object as input in the constructor.
/// It provides methods for applying filters, sorting, and includes synchronously.
/// </summary>
/// <typeparam name="T"></typeparam>
internal class ActionsService<T>(IQueryable<T> query)
{
  private IQueryable<T> _query = query;

  /// <summary>
  /// Applies the provided filter predicates to the internal IQueryable object.
  /// </summary>
  ...
  /// <returns>An IQueryable object of type T representing the filtered query.</returns>
  public IQueryable<T> ApplyFilters(Expression<Func<T, bool>>[]? predicates)
  {
  ...
  /// <summary>
  /// Applies sorting to an existing IQueryable data source synchronously.
  /// </summary>
  ...
  public IQueryable<T> ApplySorting(Expression<Func<T, object>>? orderBy, Expression<Func<T, object>>? thenBy)
  {
  ...
  /// <summary>
  /// Applies eager loading of related entities to an existing IQueryable data source synchronously.
  /// </summary>
  /// <typeparam name="T">The type of the data in the IQueryable source.</typeparam>
  /// <param name="includes">
  ...
  public IQueryable<T> ApplyIncludes(Func<IQueryable<T>, IQueryable<T>>[]? includes, Func<IQueryable<T>, IQueryable<T>>? thenInclude)
  {
